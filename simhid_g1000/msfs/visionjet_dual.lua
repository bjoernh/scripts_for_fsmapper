...
function context.start(config, aircraft)
    local display = config.simhid_g1000_display or 2
    local display3 = config.simhid_g1000_display3 or 3
    local scale = config.simhid_g1000_display_scale or 1
    ...
    viewport_afcs:register_view(context.afcs_view.create_view("Vision Jet AFCS"))

    local viewport_main_1 = mapper.viewport{
        name = "Vision Jet Main Viewport 1",
        displayno = display3,
        x = 0, y = 2/ 12 * scale,
    ...
    context.views_1 = {
        viewport_main_1:register_view(context.g3000_view.create_view("G3000 PFD", "PFD_1")),
        viewport_main_1:register_view(context.tsc_view.create_view("G3000 TSC 1", "WTG3000_GTC_1", "1")),
    }

    local viewport_main_2 = mapper.viewport{
        name = "Vision Jet Main Viewport 2",
        displayno = display,
        x = 0, y = 2/ 12 * scale,
        width = scale, height = 10 / 12 * scale,
        aspect_ratio = 16 / 10,
    ...
    context.views_2 = {
        viewport_main_2:register_view(context.g3000_view.create_view("G3000 MFD", "MFD")),
        viewport_main_2:register_view(context.tsc_view.create_view("G3000 TSC 2", "WTG3000_GTC_2", "2")),
    }

    context.current_view_1 = 1
    context.current_view_2 = 1
    
    local function change_view_1(d)
        context.current_view_1 = context.current_view_1 + d
        if context.current_view_1 > #context.views_1 then
            context.current_view_1 = 1
        elseif context.current_view_1 < 1 then
            context.current_view_1 = #context.views_1
        end
        viewport_main_1:change_view(context.views_1[context.current_view_1])
    end

    local function change_view_2(d)
        context.current_view_2 = context.current_view_2 + d
        if context.current_view_2 > #context.views_2 then
            context.current_view_2 = 1
        elseif context.current_view_2 < 1 then
            context.current_view_2 = #context.views_2
        end
        viewport_main_2:change_view(context.views_2[context.current_view_2])
    end

    viewport_main_1:set_mappings{
        {event=g1000.AUX1D.down, action=function () change_view_1(1) end},
        {event=g1000.AUX1U.down, action=function () change_view_1(-1) end},
        ...
        -- Auto Throttle
        {event=g1000.SW14.down, action=msfs.mfwasm.rpn_executer("(>K:SF50_AUTOPILOT_AUTOTHROTTLE_ARM_TOGGLE)")},
        {event=g1000.SW15.down, action=msfs.mfwasm.rpn_executer("(>K:SF50_AUTOPILOT_FMS_SPEED_MODE_PUSH)")},
        {event=g1000.SW16.down, action=msfs.mfwasm.rpn_executer("1 (>B:SF50_AUTOPILOT_Man_Speed_Mode_Toggle)")},
    ...
    viewport_main_2:set_mappings{
        {event=g1000.AUX2D.down, action=function () change_view_2(1) end},
        {event=g1000.AUX2U.down, action=function () change_view_2(-1) end},
    }
