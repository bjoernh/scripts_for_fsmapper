...
local function start(config)
    ...
    if g1000.SW26.longpressed then
        pfd_maps[#pfd_maps + 1] = {event=g1000.SW26.longpressed, action=msfs.mfwasm.rpn_executer("(>H:AS1000_PFD_COM_Switch_Long)")}
    end
    if g1000.SW31.longpressed then
        pfd_maps[#pfd_maps + 1] = {event=g1000.SW31.longpressed, action=msfs.mfwasm.rpn_executer("(>H:AS1000_PFD_CLR_Long)")}
    end
    ...
    if g1000.SW26.longpressed then
        mfd_maps[#mfd_maps + 1] = {event=g1000.SW26.longpressed, action=msfs.mfwasm.rpn_executer("(>H:AS1000_MFD_COM_Switch_Long)")}
    end
    if g1000.SW31.longpressed then
        mfd_maps[#mfd_maps + 1] = {event=g1000.SW31.longpressed, action=msfs.mfwasm.rpn_executer("(>H:AS1000_MFD_CLR_Long)")}
    end
    ...
    local pfd = viewport:register_view({
        name = "PFD",
        elements = {{object = mapper.captured_window({name = "G1000 PFD", window_title="AS1000_PFD_1"})}},
        mappings = pfd_maps,
    })
