...
local function start(config)
    g1000_context.device = common.open_simhid_g1000{
        ...
        modifiers = {
            ...
            {name = "EC3", modtype = "incdec"},
            {name = "EC3P", modtype = "button"},
            {name = "EC4X", modtype = "incdec"},
            {name = "EC4Y", modtype = "incdec"},
            {name = "EC4P", modtype = "button"},
            {name = "EC9X", modtype = "incdec"},
            {name = "EC9Y", modtype = "incdec"},
            {name = "EC9P", modtype = "button"},
        },
    }
    ...
    local pfd_maps = {
        ...
        {event=g1000.EC9X.increment, action=msfs.mfwasm.rpn_executer("(>K:AP_SPD_VAR_INC)")},
        {event=g1000.EC9X.decrement, action=msfs.mfwasm.rpn_executer("(>K:AP_SPD_VAR_DEC)")},
        {event=g1000.EC9Y.increment, action=msfs.mfwasm.rpn_executer("(>K:AP_SPD_VAR_INC)")},
        {event=g1000.EC9Y.decrement, action=msfs.mfwasm.rpn_executer("(>K:AP_SPD_VAR_DEC)")},
        {event=g1000.EC9P.down, action=msfs.mfwasm.rpn_executer("(>B:INSTRUMENT_FMS_MAN_Toggle)")},

        {event=g1000.SW26.down, action=msfs.mfwasm.rpn_executer("(>H:GEAR_TOGGLE)")},
    ...
    local mfd_maps = {
        ...
        {event=g1000.EC9X.increment, action=msfs.input_event_executer("AS1000_MFD_1_FMS_Inner", 1)},
        {event=g1000.EC9X.decrement, action=msfs.input_event_executer("AS1000_MFD_1_FMS_Inner", -1)},
        {event=g1000.EC9Y.increment, action=msfs.input_event_executer("AS1000_MFD_1_FMS_Outer", 1)},
        {event=g1000.EC9Y.decrement, action=msfs.input_event_executer("AS1000_MFD_1_FMS_Outer", -1)},
    ...
    local pfd = viewport:register_view({
        name = "PFD",
        elements = {{object = mapper.captured_window({name = "PC24 PFD", window_title="EPIC2PFD_1"})}},
        mappings = pfd_maps,
    })
    local mfd = viewport:register_view({
        name = "MFD",
        elements = {{object = mapper.captured_window({name = "PC24 Upper MFD", window_title="EPIC2UPPERMFD_3"})}},
        mappings = mfd_maps,
    })
    local lower_mfd = viewport:register_view({
        name = "Lower MFD",
        elements = {{object = mapper.captured_window({name = "PC24 Lower MFD", window_title="EPIC2LOWERMFD_2"})}},
        mappings = mfd_maps,
    })
    local tsc = viewport:register_view({
        name = "TSC",
        elements = {{object = mapper.captured_window({name = "PC24 TSC", window_title="EPIC2TSC"})}},
        mappings = mfd_maps,
    })

    local views = {pfd, mfd, lower_mfd, tsc}
    local current_view_index = 1

    local function change_view(delta)
        current_view_index = current_view_index + delta
        if current_view_index > #views then
            current_view_index = 1
        elseif current_view_index < 1 then
            current_view_index = #views
        end
        viewport:change_view(views[current_view_index])
    end
